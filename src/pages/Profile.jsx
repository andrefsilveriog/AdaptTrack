...
const DEFAULT_LIFTS = DEFAULT_EXERCISES
...
export default function Profile() {
  ...
  const initialLiftNames = (Array.isArray(profile.exerciseNames) && profile.exerciseNames.length === 3)
    ? profile.exerciseNames
    : (Array.isArray(profile.liftNames) && profile.liftNames.length === 3) ? profile.liftNames : DEFAULT_LIFTS
  ...
  const [endCurrentDate, setEndCurrentDate] = useState(todayIso())
  ...
  useEffect(() => {
    ...
    setDobIso(profile.dobIso || '')
    setSex(profile.sex || '')
    setHeight(profile.height || '')
    setTriple(!!profile.triplemeasurements)

    const ln = (Array.isArray(profile.exerciseNames) && profile.exerciseNames.length === 3)
      ? profile.exerciseNames
      : (Array.isArray(profile.liftNames) && profile.liftNames.length === 3)
        ? profile.liftNames
        : DEFAULT_LIFTS
    setLift1(ln[0])
    setLift2(ln[1])
    setLift3(ln[2])
  }, [profile.name, profile.dobIso, profile.sex, profile.height, profile.triplemeasurements, profile.exerciseNames, profile.liftNames])
  ...
  async function saveProfile(e) {
    ...
    try {
      ...
      await updateProfile({
        ...
        dobIso: String(dobIso || ''),
        sex,
        height: h,
        triplemeasurements: !!triple,
        exerciseNames: liftNames,
      })
  ...
  async function onStartCycle(e) {
    ...
    try {
      ...
      if (cycleEnd && cycleEnd < cycleStart) throw new Error('End date cannot be before the start date.')
      ...
      await startCycle(user.uid, { type: cycleType, startDateIso: cycleStart, endDateIso: cycleEnd || null, targetWeightKg })
      setCycleTarget('')
      setCycleEnd('')
      setCycleEnd('')
  ...
  async function onEndCycle(cycleId) {
    ...
    setBusy(true)
    try {
      const endIso = String(endCurrentDate || '') || todayIso()
      await endCycle(user.uid, cycleId, endIso)
            ...
            {activeCycle && (
              <>
                <div className="field" style={{ margin: 0, minWidth: 180 }}>
                  ...
                  <input type="date" value={endCurrentDate} onChange={(e) => setEndCurrentDate(e.target.value)} />
                </div>
                ...
                cycles.map((c) => {
                  ...
                  return (
                    <tr key={c.id}>
                      ...
                      <td>
                        {isActive ? (
                          <button className="btn" type="button" onClick={() => onEndCycle(c.id)} disabled={busy}>End</button>
                        ) : (
