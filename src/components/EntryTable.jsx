...
import { buildDerivedSeries, oneRepMaxKg } from '../utils/calculations.js'
import { buildCsvRows, downloadCsv } from '../utils/csv.js'
...
export default function EntryTable({ sex = 'male', userId, entries = [], tripleEnabled, exerciseNames, derived, profile }) {
  ...
  const ln = Array.isArray(exerciseNames) && exerciseNames.length === 3 ? exerciseNames : ['Bench Press', 'Squat', 'Deadlift']

  function exportCsv() {
    try {
      const d = derived || buildDerivedSeries(entries, profile || {})
      const rows = buildCsvRows(d, entries, { ...(profile || {}), exerciseNames: ln })
      const stamp = new Date().toISOString().slice(0, 10)
      downloadCsv(rows, `body-recomp-entries_${stamp}.csv`)
    } catch (e) {
      window.alert(e?.message || 'Failed to export CSV.')
    }
  }
    ...
    <div className="panel">
      <div className="panel-header">
        ...
        <div className="table-actions">
          <button className="btn small" type="button" onClick={exportCsv} disabled={!entries || entries.length === 0 || saving}>
            export CSV
          </button>
          <button
