...
export async function startCycle(userId, { type, startDateIso, endDateIso = null, targetWeightKg }) {
  if (!userId) throw new Error('Not authenticated')
  if (!isValidType(type)) throw new Error('Invalid cycle type')
  type = normalizeType(type)
  if (!startDateIso) throw new Error('Start date is required')

  // Optional: allow creating a pre-defined cycle with an end date.
  let endIsoInit = (endDateIso === '' || endDateIso === undefined) ? null : endDateIso
  if (endIsoInit && String(endIsoInit) < String(startDateIso)) {
